    ...
    class TrueRangeIndicator(BaseIndicator):
        ...
        def __init__(self, name: str = "true_range"):
            ...
            config = IndicatorConfig(
                ...
                source=IndicatorSource.CUSTOM,
                columns=["true_range"],
                description="True Range - максимальный диапазон цены"
            )
...
with nb.error_handling("Testing IndicatorFactory"):
    ...
    tr_from_factory = IndicatorFactory.create_indicator("true_range")
    pr_from_factory = IndicatorFactory.create_indicator("price_range")
...
# --- Шаг 7: Тестирование кэширования ---
nb.step("Шаг 7: Тестирование кэширования")
...
with nb.error_handling("Testing caching functionality"):
    nb.info("7.1. Тестирование кэширования в TrueRangeIndicator:")
    
    # Создаем индикатор с включенным кэшированием
    tr_cached = TrueRangeIndicator("cached_tr")
    tr_cached.enable_cache(True)
    
    nb.log(f"Кэширование включено: {tr_cached._cache_enabled}")
    
    nb.info("7.2. Первый расчет (без кэша):")
    
    # Первый расчет - должен быть медленнее
    start_time = datetime.now()
    result1 = tr_cached.calculate_with_cache(df_sample)
    time1 = (datetime.now() - start_time).total_seconds()
    
    nb.log(f"Первый расчет завершен за {time1:.4f} секунд")
    nb.log(f"Размер кэша: {len(tr_cached._cache)}")
    
    nb.info("7.3. Второй расчет (с кэша):")
    
    # Второй расчет - должен быть быстрее благодаря кэшу
    start_time = datetime.now()
    result2 = tr_cached.calculate_with_cache(df_sample)
    ...
    nb.info("7.4. Очистка кэша:")
    
    # Очищаем кэш
    tr_cached.clear_cache()
    nb.log(f"Кэш очищен. Размер кэша: {len(tr_cached._cache)}")
...
# --- Шаг 8: Создание комплексного индикатора ---
nb.step("Шаг 8: Создание комплексного индикатора")
...
with nb.error_handling("Creating complex indicator"):
    nb.info("8.1. Создание комплексного индикатора 'Price Analysis':")
    
    class PriceAnalysisIndicator(BaseIndicator):
        """
        ...
        def __init__(self, name: str = "price_analysis"):
            ...
            config = IndicatorConfig(
                ...
                parameters={},
                source=IndicatorSource.CUSTOM,
                columns=["true_range", "price_range_pct", "price_change_pct", "volatility"],
                description="Комплексный анализ цен с множественными метриками"
            )
            super().__init__(name, config)
    ...
    nb.info("8.2. Тестирование комплексного индикатора:")
...
# --- Шаг 9: Тестирование валидации данных ---
nb.step("Шаг 9: Тестирование валидации данных")
...
with nb.error_handling("Testing data validation"):
    nb.info("9.1. Тестирование валидации корректных данных:")
    ...
    nb.info("9.2. Тестирование валидации некорректных данных:")
    ...
    nb.info("9.3. Тестирование валидации данных с недостаточным количеством записей:")
...
# --- Шаг 10: Анализ архитектуры и производительности ---
nb.step("Шаг 10: Анализ архитектуры и производительности")
...
with nb.error_handling("Analyzing architecture and performance"):
    nb.info("10.1. Анализ архитектуры индикаторов:")
    ...
    for name, indicator_class in indicators_created.items():
        ...
        nb.log(f"    * Источник: {indicator_class().config.source.value}")
        nb.log(f"    * Выходные колонки: {len(indicator_class().config.columns)}")
        nb.log(f"    * Минимальные записи: {indicator_class().get_min_records()}")
    
    nb.info("10.2. Оценка производительности:")
    ...
    nb.info("10.3. Анализ использования памяти:")
...
nb.info("Мы протестировали все основные компоненты базовой архитектуры индикаторов BQuant:")
nb.log("✅ IndicatorSource - источники индикаторов")
nb.log("✅ IndicatorConfig - конфигурация индикаторов")
nb.log("✅ IndicatorResult - результаты вычислений")
nb.log("✅ BaseIndicator - базовый класс для всех индикаторов")
nb.log("✅ PreloadedIndicator - базовый класс для встроенных индикаторов")
nb.log("✅ IndicatorFactory - фабрика для создания и управления индикаторами")
...
nb.info("Базовая архитектура индикаторов BQuant предоставляет:")
nb.log("🏗️ Гибкую систему создания пользовательских индикаторов")
nb.log("🏗️ Встроенную валидацию данных и обработку ошибок")
nb.log("🏗️ Систему кэширования для оптимизации производительности")
nb.log("🏗️ Фабрику для централизованного управления индикаторами")
nb.log("🏗️ Единообразный интерфейс для всех типов индикаторов")

nb.info("Это демонстрирует мощь и гибкость архитектуры BQuant для создания технических индикаторов.")
