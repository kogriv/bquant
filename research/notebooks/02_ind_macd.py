...
Содержание:
1. Почему ушли от MACD-specific класса (исторический контекст; класс удалён в v0.1.0)
...
# =============================================================================
# ШАГ 2: ПОЧЕМУ УШЛИ ОТ MACD-SPECIFIC КЛАССА (исторический контекст)
# =============================================================================

nb.step("Шаг 2: Почему ушли от MACD-specific класса")

nb.info("Раньше был класс MACDZoneAnalyzer — удалён в v0.1.0 (был deprecated с v2.1).")
nb.info("Его ограничения — причина перехода на универсальный pipeline:")
nb.log("  [-] Работал ТОЛЬКО с MACD (нет универсальности)")
...
nb.log("")
nb.log("Теперь тот же анализ — одной строкой через пресет пайплайна:")
nb.log("  from bquant.analysis.zones import analyze_macd_zones")
nb.log("  result = analyze_macd_zones(df)")
...
nb.log("  [-] Удалён — в пакете его больше нет")
...
nb.log("  * Старый API больше не поддерживается — он удалён")
