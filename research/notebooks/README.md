...
- **2025-10-27:** ручной перезапуск скрипта `00_logging_demo.py` с параметром `--no-trap` (см. подробности ниже).【b99ea2†L1-L5】【11ebbe†L1-L11】

### Быстрая сводка (проверка 2025-10-27)

- ⚠️ `00_logging_demo.py` требует обновления: при запуске с `PYTHONPATH=.` и `--no-trap` возникают ошибки импорта `calculate_macd` и последующие сбои анализа зон, хотя процесс завершается с кодом 0.【c2d847†L1-L16】【5184a1†L1-L2】【942f52†L1-L59】【11ebbe†L1-L11】【425003†L1-L3】
...
> ℹ️ Фактические прогоны 2025-10-27 пока проведены только для `00_logging_demo.py` и показали расхождения с отчётом. После ретеста каждой записи таблица будет обновлена.

### Детальная таблица

| # | Скрипт | Категория | Статус | Примечания |
|---|--------|-----------|--------|------------|
| 1 | `00_logging_demo.py` | Data | ⚠️ Ошибки в логе (2025-10-27) | Требуется актуализировать вызов MACD и импорт `pandas`; выход 0, но этапы 2-3 завершаются с ошибками.【c2d847†L1-L16】【11ebbe†L1-L6】 |
| 2 | `01_data.py` | Data | ✅ Работает (2025-10-22) | Exit code 0, исправлены отступы【F:devref/gaps/zo/zonan.md†L3831-L3831】 |
| 3 | `01_data_loader.py` | Data | ✅ Работает (2025-10-22) | Exit code 0, все тесты OK【F:devref/gaps/zo/zonan.md†L3832-L3832】 |
| 4 | `01_data_processor.py` | Data | ✅ Работает (2025-10-22) | Exit code 0, эмодзи заменены【F:devref/gaps/zo/zonan.md†L3833-L3833】 |
...
#### Data Processing
Все шесть скриптов прошли перезапуск после очистки от проблем с отступами и эмодзи. `01_data.py` и `01_data_processor.py` теперь корректно работают на Windows и Linux.【F:devref/gaps/zo/zonan.md†L3820-L3898】 `00_logging_demo.py` сохраняет роль примера конфигурации логирования, но после запуска 2025-10-27 требует актуализации логики шага с MACD и повторного импорта `pandas`.【c2d847†L1-L16】【11ebbe†L1-L6】
...
- `00_logging_demo.py` использует устаревший импорт `calculate_macd` и полагается на `pandas`, импортируемый внутри неуспешного блока. После провала блока №2 объект `pd` недоступен для анализа зон, что приводит к дополнительной ошибке, несмотря на успешный завершение процесса с кодом 0.【c2d847†L1-L16】【11ebbe†L1-L6】
...
- Отсутствие локальной библиотеки TA-Lib вызывает многочисленные информационные предупреждения, но не прерывает выполнение скрипта.【c661f6†L1-L49】
