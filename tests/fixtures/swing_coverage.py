...
Recovered from `research/notebooks/test_global_swing_coverage.py`, which was removed
as research scratch during the OSS cleanup (batch C2). The module is pure test
infrastructure — it builds on `tests.fixtures` — so its home is here, next to
`swing_mocks.py`. Used by `tests/integration/test_pipeline_global_swings.py`.
"""
...
from tests.fixtures.swing_mocks import fake_zigzag_indicator_context
