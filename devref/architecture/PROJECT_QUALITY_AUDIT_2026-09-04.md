...
## 0. Статус исполнения (дописано 2026-09-07)

Документ ниже — **снимок на 2026-09-04** и таким и остаётся: он описывает состояние,
которое уже исправлено, и читать его надо как протокол находок, а не как описание пакета
сегодня. Заключение (§16) говорит «следующий цикл должен начинаться с P0» — этот цикл
прошёл.

| этап §12 | что | статус |
|---|---|---|
| 1. Остановить недостоверные результаты (P0) | кэш-ключ, causality, validation, pandas 3 | ✅ G53–G59, релиз 0.0.12 (2026-09-05) |
| 2. Сделать API честным (P1) | round-trip, ATR, схемы, `create_analyzer` | ✅ G59–G63, G68, релиз 0.0.13 (2026-09-06) |
| 3. Унифицировать расчёты (P2) | один TR/ATR, один warm-up, один путь индикатора | ✅ G65, релиз 0.0.14 (2026-09-06) |
| 4. Восстановить слои (P2) | фабрики из `core`, схема из `data`, виз без вычислений | ✅ G64, релиз 0.0.14 |
| 5. Усилить доказательства | CI-матрица, property-based инварианты, edge-cases | ◐ **частично, 2026-09-09**: edge-cases сделаны (G72), различимость рычагов — (G71); CI отложен решением владельца; property-based не начат — см. ниже |
| 6. Очистить репозиторий (P3) | мёртвый код, экспорты, артефакты | ✅ G67, релиз 0.0.14 |

Каждая находка воспроизведена замером **до** правки, каждый сторож проверен мутацией.
Разборы — `g64_layers_point_one_way.md`, `g65_one_mechanism_one_implementation.md`,
`g66_contracts_are_checked.md`, `g67_hygiene_holds.md` в этом каталоге и записи G53–G63,
G68 в `../gaps/`.

**Отклонено владельцем 2026-09-06, с причиной, а не забыто:**

* **AQ-039** (часть) — разнести `analysis.zones` на осцилляторы и уровни цены: зона —
  открытая абстракция, деление на два вида убило бы замысел. Разбиение god-модулей из той
  же записи выполнено.
* **AQ-046** — вынос опциональной базы в extras.
* Перепись истории git ради имён личных окружений (из §9 / этапа 6).

**Что из этапа 5 приехало попутно:** реальные round-trip-тесты (G56), тесты зависимостей
кэша (G53, AQ-044 в G66), инварианты моделей как проверки конструктора (G66, но не
property-based), снятие vacuous assertions по ходу G65–G67.

**Этап 5 отложен решением владельца 2026-09-07 — с причиной, а не по недосмотру.**
CI в репозитории нет (ни `.github/workflows`, ни `.gitlab-ci.yml`), и заводить его сейчас
признано неокупаемым: внешних контрибьюторов нет, PR от чужих нет, а релизный гейт
(чистый клон, два плеча pandas, батарея 36 скриптов, контракт архитектуры, установочная
проверка из PyPI) строже типичного CI. Классическая ценность CI — «проверить чужой патч
до мержа» — на этом проекте пока не возникает.

Что при этом сознательно принято как цена: **гейт запускается только на релизе**, поэтому
дрейф зависимостей снаружи обнаруживается в момент выпуска, а не заранее. Именно так нашёлся
AQ-008/G50 — `pandas>=2.3.0` без верхней границы, чистый клон разрешил 3.0.5 и дал 57 failed
/ 33 errors; нашлось случайно и уже на релизе. Следующий такой случай снова придёт в самый
дорогой момент. Единственная форма CI, которая это закрывала бы, — один запланированный
прогон в неделю на свежем разрешении зависимостей; она и будет первой, если решение
пересмотрят.

**Условие пересмотра, названное владельцем:** появление внешних потребителей помимо лабы
(`kogriv/bquearch`, которая пинит версию и проверяет у себя сама).

**Что из этапа 5 сделано после этой шапки (обновлено 2026-09-09):**

* **edge-cases — закрыты (G72).** Восемь странных входов прогнаны через флагманский путь
  разом; три отвечали неправильно (кадр вперемешку и дубли падали тремя слоями ниже
  сообщением про зону, `NaN` в цене молча менял число). Введён контракт входа.
* **Различимость параметров — закрыта на флагмане (G71)**, критерий §15.12. Таблица из
  15 рычагов по два прогона, и она сразу нашла своё: `analyze(regression=True)` был
  неотличим от выключенного флага.
* **CI — отложен решением владельца** (выше).

**Остался ровно один пункт этапа: property-based инварианты.** Он не зависит ни от
решения про CI, ни от появления внешних потребителей. Заведён отдельной задачей —
`property_based_invariants_backlog_2026-09.md`, чтобы не жить намерением в прозе.

**Критерии §15:** пункты 1–9 и 11 закрыты правками цикла, каждый со сторожем; 12 закрыт
частично (различающие тесты добавлялись адресно, сплошной проверки «каждый публичный
параметр» нет); **10 — «`tests/STATUS.md`, gap inventory и changelog согласованы» — был
нарушен самим этим циклом**: `tests/STATUS.md` до 2026-09-07 называл G53–G59 открытыми,
хотя они закрыты 4–5 сентября. Починено в том же проходе, что и эта шапка — и в тот же
день снова разошлось, когда закрыли G26. Повторная сверка и измеренный ответ «что ещё
нужно, чтобы стать источником доверенных выводов» —
`PRODUCTION_READINESS_2026-09-07.md` (G70, G71; не второй аудит).
