...
| 5. **Усилить доказательства** | **CI-матрица, property-based инварианты, edge-cases** | 🟠 **не начат** — см. ниже |
...
**Что из этапа 5 открыто и является главным оставшимся долгом проекта:** **CI нет** — ни
`.github/workflows`, ни `.gitlab-ci.yml`. Двуплечевой прогон (нижняя поддерживаемая и
текущая версия pandas) выполняется вручную перед релизом; критерий §15.2 держится на том,
что о нём помнят. Также не сделаны property-based инварианты и явные edge-cases (пустой
кадр, дубли, перекрытия, таймзоны, константный ряд).
