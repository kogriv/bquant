...
**Статус:** ✅ **закрыт** — уборка выполнена, дальше проект пошёл релизами и циклом качества.
Отметка поставлена 2026-09-07; последняя запись прогресса (§8) заканчивалась «осталось: релиз
0.0.3», а на тот день опубликована 0.0.15. Документ читать как исторический план уборки,
не как описание текущего состояния — см. `../architecture/package_roadmap_2026-07.md` §1.
**Одно решение из §8 позже отменено:** мягкая деградация `ZigZagSwingStrategy.calculate_global()`
(try/except → пустой контекст + WARNING) снята в G54/AQ-004 — молчаливый переход на другой
алгоритм оказался опаснее падения, и теперь неудавшийся глобальный проход это неудавшийся прогон.
...
- **D1.** ✅ Сделан 2026-07-24 (см. §8). Свинг-реконсиляция (A) + триаж 37 валидаторов (B):
...
- 2026-07-24 — **D1 сделан (части A+B).**
  **A (свинг-реконсиляция):** `user_guide/swing_strategies.md` был сломан насквозь
  (`with_swing_strategy()` не существует; выдуманные параметры; `get_zone_swings(strategy=)`;
  `ZoneInfo.metadata`) → переписан в корректный канон, весь API сверен с `bquant/analysis/zones/`.
  `analytics/zones/swing.md` (корректный, но узкий) влит в канон и удалён (+ toctree); `analytics/`
  держит comparison-case-study. Вычищены висящие ссылки на `validate_swing_pivots.py` (C2)
  и архивный `strat_issue.md` в живом `devref/gaps/swing/README.md` → указатель на smoke-тест.
  Коммит `8bce9db`.
  **B (триаж):** прогон всех 36 валидаторов `zodoctest` — 26 зелёных / 10 падений, все
  диагностированы (`TRIAGE_2026-07-24.md`). 2 реальных бага доки починены (strategies.md →
  архивный путь + стейл-хардкод валидатора; user_guide/README → `migration/MIGRATION_v2.md`);
  стало 28 зелёных. **Ключевой вывод: «все 37» не выдержала** — env-зависимые (sphinx-build/pip),
