...
**Статус:** 📦 **исторический** (отметка 2026-09-07). Работа, ради которой заведён реестр,
завершена — см. родителя `oss_readiness_plan_2026-07.md`. Документ читать как снимок июля
2026 и как источник рекомендаций **того времени**: часть пунктов исполнена, часть решена
иначе, часть отменена, и построчной сверки здесь нет. Что стало с §11 («решения, нужные от
тебя»): `zodoctest/` ушёл в архив (`../gaps/zo/zodoctest_archive/`, 2026-09-06), а не в
`tests/`; `research/` в пакете остался и описан в `research/README.md`; `CLAUDE.md`,
`GEMINI.md`, `VIBE.md` оставлены тонкими указателями на `AGENTS.md`; root clutter (§8)
дочищен 2026-09-07 — `zoval_check.md` удалён из корня как более старая копия файла из
`../gaps/zo/`.
