...
**Статус: ЗАКРЫТ 2026-08-27.** Взят вариант, которого в списке ниже не было, и он
проще обоих предложенных: убран **второй перевод**, а не вторая проверка.

`get_data_path` принимает универсальное имя таймфрейма и само переводит его в словарь
провайдера — перевод в `load_symbol_data` был лишним, и в `get_data_path` уезжало уже
переведённое `H1`, то есть значение чужого словаря под именем параметра, которое о
словаре ничего не говорит. Валидация честно сообщала «Unsupported timeframe: H1»; её
закомментировали, то есть выключили сигнал вместо причины.

Теперь `load_symbol_data` передаёт универсальное имя, а валидация в `get_data_path`
вернулась. Пути не изменились — сверено на пяти сочетаниях провайдер × таймфрейм.
Прежний результат совпадал **по случайности**: второй перевод это `.get(x, x)`, который
проваливается насквозь, когда переведённого имени нет среди ключей; стоило бы любому
провайдеру завести ключ, совпадающий с чужим выходом, и путь молча уехал бы.

Ни вариант А (убрать валидацию совсем), ни вариант Б (флаг `_skip_validation`) не
понадобились: оба принимали двойной перевод как данность и спорили лишь о том, как
пережить его последствия. Закреплено в
`tests/unit/test_core_modules.py::test_data_path_takes_the_universal_timeframe_not_the_providers`.

Отмечено попутно, **не чинилось**: `validate_timeframe` бросает голый `ValueError`, а не
`ConfigurationError` из иерархии `bquant.core.exceptions`, вопреки правилу AGENTS.md
(в `core/config.py` таких мест пять). Смена типа исключения ломает `except ValueError`
у вызывающих — отдельная правка.
