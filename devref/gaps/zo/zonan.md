...
1. [x] Расширить `bquant/visualization/zones.py` (~250 строк)
   - [x] Метод `plot_zone_detail()` → См. [Детальный график зоны (plot_zone_detail)](#визуализация)
   - [x] Метод `plot_zones_comparison()` → См. [Сравнение зон (plot_zones_comparison)](#визуализация)
   - [x] Helper `_get_zone_window()` → См. [Детальный график зоны](#визуализация)
   - [x] Helper `_detect_indicators_from_features()` → См. [Детальный график зоны](#визуализация)
   - [x] Helper `_filter_zones_by_date()` - фильтрация по дате
   - [x] Plotly реализации для обоих методов
   - [x] Matplotlib реализации (упрощенные)

2. [x] Обновить `bquant/analysis/zones/models.py` (~50 строк)
   - [x] Добавить метод `ZoneAnalysisResult.visualize()` → См. [Интеграция с ZoneAnalysisResult](#визуализация)
   - [x] Поддержка режимов: overview, detail, comparison, statistics

3. [x] Обновить `bquant/visualization/__init__.py`
   - [x] Экспортировать новые методы визуализации
   - [x] Обновить convenience функции

4. [x] Unit-тесты визуализации (~15 тестов)
   - [x] Тесты `plot_zone_detail()` для разных конфигураций
   - [x] Тесты `plot_zones_comparison()` с фильтрами
   - [x] Тесты `ZoneAnalysisResult.visualize()` для всех режимов
   - [x] Тесты auto-detect индикаторов

5. [ ] Примеры визуализации → См. [Примеры использования визуализации](#визуализация) и [Сценарий 10](zomodul.md)
   - [x] Добавить в examples/ пример детальной визуализации → `examples/09_zones_visualization_demo.py` ✅
   - [ ] Notebook с различными режимами визуализации
   - [ ] Screenshots для документации

**Текущий статус (2025-10-28):** Основные реализации визуализации завершены (zones.py, интеграция в models.py и __init__.py, unit-тесты). Создан полнофункциональный пример `examples/09_zones_visualization_demo.py` с демонстрацией всех режимов визуализации. Остаётся добавить notebook с режимами и скриншоты в документацию.
