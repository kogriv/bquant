...
1. [ ] API документация → Использовать заготовки из zonan.md
   - ✅ Переписаны `docs/api/analysis/zones.md`, `strategies.md`, `pipeline.md` под Universal Pipeline (см. валидацию `docs/api` в `zoval_check.md`).
   - ⚠️ Исправить legacy-пример `find_support_resistance` в `docs/api/analysis/zones.md` — падает `AttributeError: 'int' object has no attribute 'total_seconds'` в `tests/docs/test_zones_validation.py`.
   - [ ] Добавить проверки/примеры для новых маркетинговых сценариев (например, `FICTIONAL_INDICATOR_99`) либо скорректировать текст, чтобы соответствовать тестам.
   - [ ] Финальная сверка ссылок/крест-референсов после исправления legacy-блока.

2. [ ] User Guide → Использовать примеры из zonan.md
   - ✅ Quick Start и основной README обновлены под Universal Pipeline.
   - ⚠️ Уточнить структуру: план ожидал `docs/user_guide/zone_analysis.md`, фактически создан `zone_analysis_pipeline.md`. Решить — переименовать файл, либо обновить план и навигацию, чтобы отражать текущее расположение.
   - [ ] Migration guide (переход со старого API) — оформить как `MIGRATION_v2.md`/раздел user guide, использовать сравнение Пример 7 (новый) vs старый `MACDZoneAnalyzer`.
   - [ ] Best practices — вынести материал из [Best Practices](zomodul.md#best-practices) в отдельный раздел user guide.

3. [ ] Developer Guide → Использовать точки расширения
   - [ ] Создать `docs/developer_guide/zone_detection_strategies.md` (шаблон + пример реализации стратегии).
   - [ ] Описать процесс добавления новой стратегии детекции (ссылки на [Точки расширения: Слой 1](#точки-расширения)).
   - [ ] Описать расширение `UniversalZoneAnalyzer` (см. [Точки расширения: Слой 2](#точки-расширения)).
   - [ ] Подготовить contribution guide по работе с универсальной архитектурой.

4. [ ] Tutorials → Адаптировать из zonan.md
...
5. [ ] CHANGELOG и migration notes
   - [ ] Обновить раздел `Unreleased` в `CHANGELOG.md` ключевыми изменениями Universal Zone Analyzer + pipeline.
...
**Текущий статус (2025-10-xx):** API и User Guide частично обновлены, остальные подпункты открыты. Этап завершён не будет, пока не закрыты оставшиеся задачи и не проходят валидационные тесты документации.
