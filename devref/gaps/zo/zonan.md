...
#### 2.4 Исследовательские ноутбуки (research/notebooks/) - 1 день
...
**✅ Проверка актуальности (2025-10-18):**

**Проверено:** 19/19 скриптов с `--no-trap` - полная валидация совместимости

| Категория | Работают | Не работают | Неполные | Итого |
|-----------|----------|-------------|----------|-------|
| Data Processing | 4/6 (67%) | 2/6 | 0/6 | 6 |
| Indicators | 6/7 (86%) | 1/7 | 0/7 | 7 |
| Analysis | 2/5 (40%) | 2/5 | 1/5 | 5 |
| Utilities | 1/1 (100%) | 0/1 | 0/1 | 1 |
| **ИТОГО** | **13/19 (68%)** | **5/19 (26%)** | **1/19 (5%)** | **19** |

**Детальная таблица:**

| # | Скрипт | Категория | Статус | Проблема |
|---|--------|-----------|--------|----------|
| 1 | `00_logging_demo.py` | Data | ✅ Работает | - |
| 2 | `01_data.py` | Data | ❌ Не работает | IndentationError (line 23) |
| 3 | `01_data_loader.py` | Data | ✅ Работает | - |
| 4 | `01_data_processor.py` | Data | ❌ Не работает | UnicodeEncodeError (эмодзи) |
| 5 | `01_data_schemas.py` | Data | ✅ Работает | - |
| 6 | `01_data_validator.py` | Data | ✅ Работает | - |
| 7 | `02_ind_base.py` | Indicators | ✅ Работает | - |
| 8 | `02_ind_calculators.py` | Indicators | ✅ Работает | - |
| 9 | `02_ind_factory.py` | Indicators | ✅ Работает | - |
| 10 | `02_ind_lib.py` | Indicators | ✅ Работает | talib unavailable (ожидаемо) |
| 11 | `02_ind_library.py` | Indicators | ✅ Работает | - |
| 12 | `02_ind_macd.py` | Indicators | ❌ Не работает | KeyError: 'fast' |
| 13 | `02_ind_types.py` | Indicators | ✅ Работает | - |
| 14 | `03_analysis_base.py` | Analysis | ✅ Работает | - |
| 15 | `03_analysis_new_features.py` | Analysis | ❌ Частично | `_zone_to_dict()` removed |
| 16 | `03_analysis_statistical.py` | Analysis | ✅ Работает | - |
| 17 | `03_analysis_zones.py` | Analysis | ❌ Не работает | Старые Zone классы |
| 18 | `03_zones.py` | Analysis | 🔨 Неполный | <50 строк |
| 19 | `bq.py` | Utilities | ✅ Работает | - |

**Ключевые проблемы:**
- **Indicators:** 1/7 не работает (MACD - устаревший API)
- **Analysis:** 3/5 проблемных (2 не работают, 1 неполный) - требуют обновления для новой архитектуры
- **Data Processing:** 2/6 с техническими ошибками (IndentationError, UnicodeEncodeError)

**Детали ошибок:**
- **`02_ind_macd.py`**: KeyError на строке 113 при обращении к `analyzer.macd_params['fast']` (теперь `'fast_period'`)
- **`03_analysis_zones.py`**: ValueError при инициализации старого `ZoneFeaturesAnalyzer` с `swing_strategy='zigzag'`
- **`03_analysis_new_features.py`**: Step 1 работает, Step 2+ падают с AttributeError (метод `_zone_to_dict()` удален)
- **`03_zones.py`**: Только заголовок и план (42 строки), без реализации
- **`01_data.py`**: Синтаксическая ошибка (неправильный отступ)
- **`01_data_processor.py`**: Проблемы с кодировкой эмодзи в Windows
...
Текущее состояние `research/notebooks/`:
- `02_ind_macd.py` (729 строк) - **❌ Не работает** → **ОБНОВИТЬ** (задача #1)
- `03_zones.py` (42 строки) - **🔨 Неполный** → **УДАЛИТЬ** (задача #3)
- `03_analysis_zones.py` (656 строк) - **❌ Не работает (старая архитектура)** → **СОХРАНИТЬ как есть** для исторической справки
- `03_analysis_new_features.py` (693 строки) - **❌ Частично работает** (Step 1 OK, Step 2+ fail) → **ПЕРЕПИСАТЬ** на новый API
...
2. [x] **Создать `research/notebooks/03_zones_universal.py`** (412 строк) ✅ **ВЫПОЛНЕНО**
   ...
   **Детализация:**
   - [ ] Использовать все примеры из [Примеры использования](#примеры)
   ...
   - [x] **Результаты:** 10 шагов, 72 MACD зоны, ~14K зон/сек, кэширование работает ✅
   - ⚠️ **Выявлен баг:** ZoneFeaturesAnalyzer hardcoded для MACD (документировано)
...
4. [x] **Обновить `research/notebooks/README.md`** ✅ **ВЫПОЛНЕНО**
   - [x] Добавлено описание 03_zones_universal.py (412 строк, 10 шагов) ✅
   - [x] Обновлен статус 02_ind_macd.py (работает, migration guide) ✅
   - [x] Обновлена сводная таблица (15/19 работают, +2) ✅
   - [x] Обновлена категоризация по назначению ✅
   - [x] Обновлены приоритеты (Stage 2.4 завершен) ✅
   - [x] Добавлены выявленные баги в список ✅

#### 2.5 Integration тесты (0.5 дня)

1. [ ] Создать `tests/integration/test_zone_analysis_e2e.py` (~200 строк)
   - [ ] End-to-end тесты всего pipeline для разных индикаторов
   - [ ] MACD: `test_macd_full_pipeline()`
   - [ ] RSI: `test_rsi_full_pipeline()`
   - [ ] AO: `test_ao_full_pipeline()`
   - [ ] Preloaded: `test_preloaded_zones_pipeline()`
   - [ ] Performance: `test_pipeline_performance()` (не медленнее старого)
   
2. [ ] Создать `tests/integration/test_backward_compatibility.py` (~100 строк)
   - [ ] Тесты что старый MACDZoneAnalyzer работает через новый API
   - [ ] Сравнение результатов старого vs нового (должны совпадать)
