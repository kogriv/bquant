...
│       │
│       ├── pipeline.py             # НОВОЕ - ZoneAnalysisPipeline + Builder
│       │
...
└── indicators/
    ...
    │
    └── macd.py                      # РЕФАКТОРИНГ - переместить:
                                     # - ZoneInfo → analysis/zones/models.py
                                     # - ZoneAnalysisResult → analysis/zones/models.py
                                     # - MACDZoneAnalyzer → УДАЛИТЬ (заменен Pipeline)
                                     # Добавить: backward compatibility alias
```
...
#### 1.1 Слой 1: Zone Detection Strategies (2-3 дня)
...
1. [ ] Создать `bquant/analysis/zones/detection/__init__.py`
   - Экспорты: ZoneDetectionStrategy, ZoneDetectionConfig, ZoneDetectionRegistry

2. [ ] Создать `bquant/analysis/zones/detection/base.py` (~100 строк)
...
3. [ ] Создать `bquant/analysis/zones/detection/registry.py` (~100 строк)
...
4. [ ] Реализовать стратегии детекции → См. [Реализация стратегий детекции](#реализация-стратегий):
   - [ ] `zero_crossing.py` (~150 строк) → См. [Стратегия 1: ZeroCrossingDetection](#реализация-стратегий)
   - [ ] `line_crossing.py` (~120 строк) → См. [Стратегия 3: LineCrossingDetection](#реализация-стратегий)
   - [ ] `threshold.py` (~130 строк) → См. [Стратегия 2: ThresholdDetection](#реализация-стратегий)
   - [ ] `preloaded.py` (~200 строк) → См. [Стратегия 4: PreloadedZonesDetection](#реализация-стратегий)
   - [ ] `combined.py` (~150 строк, опционально) → См. [Стратегия 5: CombinedRulesDetection](#реализация-стратегий)

5. [ ] Unit-тесты стратегий детекции (~50 тестов)
   - По 10 тестов на каждую стратегию
   - Тесты: валидация конфигурации, обработка ошибок, корректность детекции

#### 1.2 Слой 2: Universal Analyzer (1 день)

**📖 Спецификация:** См. раздел [Слой 2: Universal Zone Analyzer](#слой-2)

1. [ ] Создать `bquant/analysis/zones/analyzer.py` (~200 строк)
...
1. [ ] Создать `bquant/analysis/zones/pipeline.py` (~500 строк)
   - [ ] Dataclass `IndicatorConfig` → См. [Pipeline: Конфигурация](#pipeline)
   - [ ] Dataclass `ZoneAnalysisConfig` → См. [Pipeline: Конфигурация](#pipeline)
   - [ ] Класс `ZoneAnalysisPipeline` → См. [Pipeline: Pipeline класс](#pipeline):
   ...
   - [ ] Класс `ZoneAnalysisBuilder` (fluent API) → См. [Builder: Реализация](#builder):
   ...
   - [ ] Helper функция `analyze_zones(df)` → См. [Builder: Реализация](#builder)

2. [ ] Обновить `bquant/analysis/zones/__init__.py`
   - [ ] Экспортировать все новые классы и функции
   - [ ] Добавить __all__ список

3. [ ] Unit-тесты Pipeline + Builder (~25 тестов)
   - [ ] Тесты конфигураций
   - [ ] Тесты Pipeline выполнения
   - [ ] Тесты Builder цепочек
   - [ ] Тесты интеграции с IndicatorFactory → См. [Интеграция с IndicatorFactory](#интеграция)
   - [ ] **Тесты кэширования (5 новых тестов)** → См. [Примеры использования кэширования](#кэширование):
     - Cache hit/miss
     - Инвалидация кэша
     - TTL expiration
     - Отключение кэша
...
**Результат:** Новая архитектура работает с кэшированием, старая не сломана, ~90 тестов проходят

### Этап 2: Миграция существующего кода (2-3 дня)

**Цель:** Обеспечить совместимость со старым кодом через новую архитектуру

**📖 Справка:** См. разделы [Примеры использования](#примеры) и [Модульное использование](zomodul.md)

1. [ ] Создать backward compatibility алиасы в `bquant/indicators/macd.py`
   - [ ] Класс `MACDZoneAnalyzer` как wrapper вокруг Pipeline
   - [ ] Метод `analyze_complete_modular()` использует новый Pipeline
   - [ ] Добавить deprecation warnings с указанием на новый API
   - [ ] Сохранить старую сигнатуру методов

2. [ ] Обновить примеры использования (examples/) → См. [Примеры использования](#примеры)
   - [ ] `examples/02_macd_zone_analysis.py` - добавить примеры нового API (Пример 1, 6, 7)
   - [ ] `examples/04_comprehensive_analysis.py` - обновить под новую архитектуру
   - [ ] Создать новый пример с другими индикаторами → См. [Пример 4 (AO)](#примеры) и [Пример 5 (RSI)](#примеры)

3. [ ] Обновить notebooks → Использовать примеры из документов
   - [ ] Найти все notebooks, использующие MACDZoneAnalyzer
   - [ ] Добавить примеры нового fluent API → См. [Пример 1](#примеры)
   - [ ] Показать сравнение старого vs нового подходов
   - [ ] Примеры модульного использования → См. [zomodul.md](zomodul.md) (Сценарии 1-12)

4. [ ] Обновить тесты MACDZoneAnalyzer (~20 тестов)
   - [ ] Тесты backward compatibility
   - [ ] Проверка deprecation warnings
   - [ ] Все старые тесты должны проходить

5. [ ] Integration тесты (~10 тестов) → Использовать примеры из документа
   - [ ] End-to-end тесты новой архитектуры → См. [Примеры 1-8](#примеры)
   - [ ] Тесты различных комбинаций индикаторов и стратегий → См. [Примеры 4, 5, 8](#примеры)
   - [ ] Performance тесты (не должно быть замедления)
   - [ ] Тесты модульного использования → См. [zomodul.md](zomodul.md)

**Результат:** Старый код работает через новую архитектуру, все тесты проходят

### Этап 3: Документация (2-3 дня)

**Цель:** Обновить документацию для пользователей и разработчиков

**📖 Источники контента:** См. все разделы zonan.md + [zomodul.md](zomodul.md)

1. [ ] API документация → Использовать заготовки из zonan.md
   - [ ] Обновить `docs/api/analysis/zones.md`
   - [ ] Добавить документацию для detection стратегий → См. [Реализация стратегий детекции](#реализация-стратегий)
   - [ ] Документация Pipeline + Builder → См. [Pipeline](#pipeline) и [Builder](#builder)
   - [ ] Примеры использования для каждого класса → См. [Примеры использования](#примеры)

2. [ ] User Guide → Использовать примеры из zonan.md
   - [ ] Обновить `docs/user_guide/zone_analysis.md`
   - [ ] Quick start с новым API → См. [Пример 1: MACD через fluent builder](#примеры)
   - [ ] Migration guide (переход со старого API) → Сравнить Пример 7 (новый) vs старый MACDZoneAnalyzer
   - [ ] Best practices → См. [Best Practices](zomodul.md#best-practices)

3. [ ] Developer Guide → Использовать точки расширения
   - [ ] `docs/developer_guide/zone_detection_strategies.md` → См. [Шаблон для новых стратегий](#реализация-стратегий)
   - [ ] Как добавить новую стратегию детекции → См. [Точки расширения: Слой 1](#точки-расширения)
   - [ ] Как расширить UniversalZoneAnalyzer → См. [Точки расширения: Слой 2](#точки-расширения)
   - [ ] Contribution guide → См. [Точки расширения архитектуры](#точки-расширения)

4. [ ] Примеры и tutorials → Адаптировать из zonan.md
   - [ ] Tutorial: анализ зон MACD → См. [Пример 1](#примеры)
   - [ ] Tutorial: анализ зон RSI → См. [Пример 5](#примеры)
   - [ ] Tutorial: кастомные правила детекции → См. [Пример 8](#примеры)
   - [ ] Tutorial: работа с preloaded зонами → См. [Пример 3](#примеры) и [Сценарий 9](zomodul.md)

5. [ ] CHANGELOG и migration notes
   - [ ] Обновить `CHANGELOG.md`
   - [ ] Создать `MIGRATION_v2.md` с подробными инструкциями
   - [ ] Список breaking changes (если есть)

**Результат:** Полная документация для пользователей и разработчиков
...
| **Этап 2** | 2-3 дня | Backward compatibility + примеры | +30 новых | Совместимость обеспечена |
| **Этап 3** | 2-3 дня | Документация | - | Полная документация |
