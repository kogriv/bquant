...
**Статус:** ✅ R1 сделан — input-guard в обёртке ZigZag; segfault устранён, полный
`pytest` доходит до конца без краша (дефолт остаётся zigzag)
...
### R1. Защитить обёртку ZigZag в bquant от вырожденного входа ✅ СДЕЛАНО
- Реализован `_is_degenerate()` в `ZigZagSwingStrategy`: вход считается вырожденным,
  если `high.nunique() < legs` или `low.nunique() < legs` (мало различных уровней для
  формирования пивотов). Guard стоит в обоих путях к numba — `calculate_global` и
  `calculate` — и возвращает пустой `SwingContext`/`_empty_metrics()` до захода в
  pandas-ta zigzag.
- Место: `bquant/analysis/zones/strategies/swing/zigzag.py`.
- Проверено: плоские данные через pipeline → 0 зон без краша; реальные данные → те же
  72 зоны; полный `pytest` доходит до конца (segfault ушёл), новых регрессий нет.
- Порог `nunique < legs` откалиброван по границе краша (ловит const high/low и
  near-const «one spike»; реальные данные с сотнями уровней не задевает).
- Плюс: защищает и **реальные** плоские сегменты (остановленный/неликвидный рынок).
