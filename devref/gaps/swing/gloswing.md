...
- [x] ✅ **ТЕСТ**: Smoke test на cache invalidation - создать fake кэш с version=1, попытаться загрузить, проверить что вернулся None
...
**Валидация гипотез**:
- [ ] ⭐ **ТЕСТ**: Smoke test на memory - создать SwingContext с 1000 SwingPoint, измерить через `sys.getsizeof()`, проверить ~264 KB ±50% (≈234 KB, укладывается в диапазон)
...
**Подготовка к будущим фазам** (⏳ non-blocking):
- [x] ⏳ Код готов к unit тестам из Фазы 5: `calculate_global()` возвращает пустой SwingContext если данных < legs*2 (не падает с исключением) — сделано: проверки размера добавлены.
