...
4.1. ✅ **Обновить `extract_zone_features()` с логикой выбора режима** (`bquant/analysis/zones/zone_features.py`)
     ...
     **Checklist**:
     - [ ] Проверка наличия swing_context в zone_info (см. раздел 5)
     - [ ] ВЕТКА GLOBAL: Создание temp_zone с swing_context (см. раздел 5)
     - [ ] ВЕТКА GLOBAL: Вызов `strategy.aggregate_for_zone(temp_zone, context)` (см. раздел 5)
     - [ ] ВЕТКА GLOBAL: Установка metadata['swing_calculation_mode'] = 'global' (см. раздел 5)
     - [ ] ВЕТКА PER_ZONE: Вызов `strategy.calculate(data)` (см. раздел 5)
     - [ ] ВЕТКА PER_ZONE: Установка metadata['swing_calculation_mode'] = 'per_zone' (см. раздел 5)
     - [ ] Логирование для обеих веток (см. раздел 5)
     - [ ] Обработка исключений (см. раздел 5)
...
4.2. ✅ **Добавить маркер `swing_calculation_mode` в metadata**
     ...
     **Checklist**:
     ...
     - [ ] Значение 'global' для глобального режима (см. раздел 5)
     - [ ] Значение 'per_zone' для legacy режима (см. раздел 5)
     - [ ] Использование в integration тестах для проверки режима

4.3. ✅ **Обновить логирование**

     **Checklist**:
     - [ ] DEBUG уровень для global mode: "Swing metrics aggregated from global context: N rallies, M drops, ratio=X" (см. раздел 5)
     - [ ] DEBUG уровень для per_zone mode: "Swing metrics calculated in per_zone mode: N rallies, M drops" (см. раздел 5)
     - [ ] WARNING при ошибках: "Failed to calculate swing metrics: {e}" (см. раздел 5)
...
**Критические тесты**:
- [ ] ✅ **ТЕСТ**: Smoke test на global режим - создать zone_info с swing_context, вызвать extract_zone_features(), проверить metadata['swing_calculation_mode'] == 'global'
- [ ] ✅ **ТЕСТ**: Smoke test на per_zone режим - создать zone_info БЕЗ swing_context, проверить metadata['swing_calculation_mode'] == 'per_zone'
- [ ] ✅ **ТЕСТ**: Smoke test на exception handling - передать битую стратегию, проверить что WARNING залогирован и metadata['swing_metrics'] == None
