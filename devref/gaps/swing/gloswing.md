...
**📖 Обязательное pre-reading**:
- **Раздел 10** - "Риски и меры" (строки 1640-1771) - все edge cases и риски для тестирования
- **Раздел 1.2** - "SwingContext.slice()" (строки 130-165) - алгоритм нарезки для тестирования границ
- **Раздел 9** - "Совместимость, кэширование и fallback" (строки 1480-1636) - сценарии для интеграционных тестов

---

**5.1. Unit-тесты основных сценариев**:
```python
...
1. **Lazy Loading** (будущая оптимизация):
   ```python
   class SwingContext:
       """
       ...
       def __init__(self, ..., lazy_load: bool = False):
           if lazy_load:
               # Хранить только indices и метаданные
               # Загружать SwingPoint по требованию из кэша
               self._lazy = True
               self._cache_path = ...
   ```

2. **Chunking для экстремальных датасетов**:
   ```python
   # Если датасет > 5M баров, разбить на чанки
   if len(data) > 5_000_000:
       logger.warning(
           f"Large dataset ({len(data)} bars), consider chunking. "
           f"Estimated memory: {estimate_swing_memory(data)} MB"
       )
   ```

3. **Memory-efficient альтернатива** (numpy structured arrays):
   ```python
   # Вместо List[SwingPoint] использовать numpy structured array
   swing_dtype = np.dtype([
       ('point_id', 'i4'),
       ('index', 'i4'),
       ('price', 'f8'),
       ('amplitude_to_next', 'f8'),
       ('duration_to_next', 'i4'),
       # timestamp и strings хранить отдельно
   ])
   # Экономия: ~50% памяти
   ```

4. **Graceful fallback при MemoryError**:
   ```python
   # В Pipeline._calculate_global_swings()
   try:
       swing_contexts = {}
...
**Рекомендации**:
- ✅ Для датасетов <100k баров: никаких ограничений
- ⚠️ Для датасетов 100k-1M баров: мониторить потребление памяти
- ❌ Для датасетов >1M баров: рассмотреть chunking или per_zone режим
