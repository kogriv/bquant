...
1.1. ✅ **Создать `SwingPoint` dataclass** (`bquant/analysis/zones/models.py`)
     ...
     **Checklist**:
     - [ ] Все 9 полей согласно таблице Attributes в спецификации раздела 1.1
     - [ ] `__post_init__` валидация для strategy_params (см. пример в разделе 1.1)
...
1.2. ✅ **Создать `SwingContext` dataclass** (`bquant/analysis/zones/models.py`)
     ...
     **Checklist**:
     - [ ] 5 полей: swing_points, indices, full_data_length, strategy_name, strategy_params (см. Attributes в разделе 1.2)
     - [ ] Метод `slice()` с bisect алгоритмом (см. раздел 1.2) - **КРИТИЧЕСКИ ВАЖНО**
     - [ ] Метод `get_swings_for_zone()` (см. раздел 1.2)
     - [ ] Метод `to_dict()` для сериализации (см. раздел 1.2)
...
1.3. ✅ **Обновить `ZoneInfo`** (`bquant/analysis/zones/models.py`)
     ...
     **Checklist**:
     - [ ] Добавить поле `swing_context: Optional[SwingContext] = None` (см. раздел 1.3)
     - [ ] Реализовать метод `get_zone_swings()` (см. раздел 1.3)
     - [ ] Обновить `to_analyzer_format()` - включить swing_context (см. раздел 1.3)
     - [ ] Docstrings с примерами использования (см. раздел 1.3)
...
1.4. ✅ **Добавить `swing_scope` в `ZoneAnalysisConfig`** (`bquant/analysis/zones/pipeline.py`)
     ...
     **Checklist**:
     - [ ] Поле `swing_scope: Literal["per_zone", "global"] = "per_zone"` (см. раздел 2)
     - [ ] Обновить `to_cache_key()` - включить swing_scope (см. раздел 2)
     - [ ] Default = "per_zone" для обратной совместимости
     - [ ] Docstring объясняет оба режима (см. раздел 2)
...
- [ ] ✅ Кэширование обновлено (CACHE_VERSION = 2, двусторонняя схема save/load)

**Критические тесты**:
- [ ] ✅ **ТЕСТ**: Запустить `mypy bquant/analysis/zones/models.py` - проверить type hints
- [ ] ✅ **ТЕСТ**: Smoke test на сериализацию - вызвать `SwingContext.to_dict()`, проверить что Dict содержит все поля
- [ ] ✅ **ТЕСТ**: Smoke test на cache invalidation - создать fake кэш с version=1, попытаться загрузить, проверить что вернулся None
...
**Валидация гипотез**:
- [ ] ⭐ **ТЕСТ**: Smoke test на memory - создать SwingContext с 1000 SwingPoint, измерить через `sys.getsizeof()`, проверить ~264 KB ±50%
