...
3.1. ✅ **Добавить `_calculate_global_swings()` в `ZoneAnalysisPipeline`** (`bquant/analysis/zones/pipeline.py`)
     ...
     **Checklist**:
     ...
     - [ ] Проверка поддержки глобального режима (см. раздел 4.1)
     - [ ] Вызов `self._swing_strategy.calculate_global(data)` (см. раздел 4.1)
     - [ ] Логирование количества обнаруженных свингов (см. раздел 4.1)
...
3.2. ✅ **Добавить `_inject_swing_context()` в `ZoneAnalysisPipeline`**
     ...
     **Checklist**:
     ...
     - [ ] Итерация по зонам: `for zone in zones: zone.swing_context = swing_context` (см. раздел 4.1)
     - [ ] Side effect: модификация in-place (см. раздел 4.1)
     - [ ] Логирование на DEBUG уровне (см. раздел 4.1)
...
3.3. ✅ **Обновить `_run_without_cache()` с новым воркфлоу**
     ...
     **Checklist**:
     - [ ] Шаг 1: prepare_dataframe() - без изменений
     - [ ] Шаг 2: Вызов _calculate_global_swings() если swing_scope=="global"
     - [ ] Шаг 2a: try-except с fallback на per_zone (см. раздел 9.3 Graceful Degradation)
     - [ ] Шаг 3: detect_zones() - без изменений
     - [ ] Шаг 4: _inject_swing_context() если контекст существует
     - [ ] Шаг 5: analyze_zones() - без изменений
...
3.4. ✅ **Добавить `with_swing_scope()` в `ZoneAnalysisBuilder`**
     ...
     **Checklist**:
     ...
     - [ ] Валидация scope: raise ValueError если не 'per_zone' или 'global' (см. раздел 4.2)
     - [ ] Установка: `self.pipeline._config.swing_scope = scope` (см. раздел 4.2)
     - [ ] Логирование: `self.logger.info(f"Swing calculation mode set to: {scope}")` (см. раздел 4.2)
     - [ ] Возврат self для fluent API (см. раздел 4.2)
     - [ ] Полный docstring с примерами (см. раздел 4.2)
...
3.5. ✅ **Обновить логирование для отслеживания режима**
     ...
     **Checklist**:
     - [ ] INFO: "Calculating global swings with strategy: ..." (см. раздел 4.1)
     - [ ] INFO: "Global swings calculated: N swing points detected" (см. раздел 4.1)
     - [ ] WARNING: "Global swing calculation failed, falling back..." (см. раздел 4.1)
     - [ ] INFO: "Swing calculation mode set to: global/per_zone" (см. раздел 4.2)
     - [ ] DEBUG: "Injected swing_context into N zones" (см. раздел 4.1)
...
**Критические тесты**:
- [ ] ✅ **ТЕСТ**: Smoke test на fallback - передать некорректные данные, проверить что WARNING залогирован и execution продолжается
- [ ] ✅ **ТЕСТ**: Smoke test на логирование - вызвать с swing_scope='global', проверить наличие INFO "Calculating global swings..."
- [ ] ✅ **ТЕСТ**: Smoke test на переключение режимов - запустить с 'per_zone', затем с 'global', убедиться что результаты отличаются
