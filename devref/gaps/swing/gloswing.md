...
**Критические тесты**:
- [ ] ✅ **ТЕСТ**: Запустить `mypy bquant/analysis/zones/models.py` - проверить type hints *(⚠️ падает из-за отсутствующих type stubs в сторонних пакетах; см. вывод mypy)*
...
2.1. ✅ **Обновить протокол `SwingCalculationStrategy`** (`bquant/analysis/zones/strategies/base.py`)
     ...
     **Checklist**:
     ...
     - [ ] Метод `calculate_global(full_data) -> SwingContext` (см. раздел 3.1)
     - [ ] Метод `aggregate_for_zone(zone, context) -> SwingMetrics` (см. раздел 3.1)
     - [ ] Метод `calculate(zone_data) -> SwingMetrics` помечен как DEPRECATED (см. раздел 3.1)
     - [ ] Метод `config_hash() -> Dict[str, Any]` (см. раздел 3.1)
...
2.2. ✅ **Реализовать `calculate_global()` для `ZigZagSwingStrategy`** (`strategies/swing/zigzag.py`)
     ...
     **Checklist**:
     - [ ] Валидация входных данных (см. раздел 3.2)
     - [ ] Применение pandas-ta ZigZag (см. раздел 3.2)
     - [ ] Обработка edge cases (недостаточно колонок, мало точек) (см. раздел 3.2)
     - [ ] Преобразование в SwingPoint объекты (см. раздел 3.2)
     - [ ] Определение swing_type (peak/trough) по направлению (см. раздел 3.2)
     - [ ] Расчёт amplitude_to_next и duration_to_next (см. раздел 3.2)
     - [ ] Возврат SwingContext с полными метаданными (см. раздел 3.2)
...
2.3. ✅ **Реализовать `aggregate_for_zone()` для `ZigZagSwingStrategy`** (`strategies/swing/zigzag.py`)
     ...
     **Checklist**:
     - [ ] Получение zone_swings через `context.get_swings_for_zone(zone)` (см. раздел 3.2)
     - [ ] Проверка минимального количества свингов (см. раздел 3.2)
     - [ ] Разделение на rallies и drops по price_change_pct (см. раздел 3.2)
     - [ ] Вызов `_aggregate_metrics()` для финальной агрегации (см. раздел 3.2)
...
2.4. ✅ **Извлечь `_aggregate_metrics()` как общий метод** (`strategies/swing/zigzag.py`)
     ...
     **Checklist**:
     ...
     - [ ] Рассчитывает ВСЕ 23 поля SwingMetrics (см. раздел 3.2)
     - [ ] Вызывает `metrics.validate()` перед возвратом (см. раздел 3.2)
...
2.7. ✅ **Обновить `_AdaptiveSwingStrategy`** (`strategies/swing/thresholds.py`)
     ...
     **Checklist**:
     - [ ] Добавить `_global_threshold_cache` поле (см. раздел 6)
     - [ ] Реализовать `calculate_global()` - пороги ONE TIME на глобальных данных (см. раздел 6)
     - [ ] Реализовать `aggregate_for_zone()` - использовать глобальные пороги (см. раздел 6)
...
- [ ] ✅ **ТЕСТ**: Smoke test для ZigZag - вызвать `calculate_global(sample_data)`, проверить что вернулся SwingContext с swing_points
- [ ] ✅ **ТЕСТ**: Smoke test для агрегации - вызвать `aggregate_for_zone(zone, context)`, проверить что вернулся SwingMetrics
