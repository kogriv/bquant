...
- [ ] **API Reference — `docs/api/...`**
  - Обновить `docs/api/indicators/README.md`: добавить подраздел о связке *LibraryManager + динамический pandas-ta loader*, включить новую страницу в локальный toctree.
  - Обновить `docs/api/indicators/base.md`: скорректировать описание `IndicatorFactory`/`LibraryIndicator`, убрать акцент на ручной регистрации функций и описать использование `create()` и библиотечных обёрток.
  - Обновить `docs/api/indicators/factory.md`: переписать список методов и примеры так, чтобы они отражали текущую реализацию (`create`, динамические классы, делегирование LibraryManager) и ссылались на новый раздел.
  - Создать отдельную страницу `docs/api/indicators/library_manager.md` с описанием публичных методов (`load_all_libraries`, `get_library_info`, `create_indicator`) и пошаговыми примерами «простого способа» получения pandas-ta индикатора.
  - Обновить `docs/api/README.md`, добавив пункт про `bquant.indicators.library`/`LibraryManager`, чтобы новая страница оказалась доступной из оглавления API.
- [ ] **Руководства и быстрый старт**
  - В `docs/user_guide/quick_start.md` вставить небольшой пример использования `LibraryManager.create_indicator()` (с пояснением, что индикаторы pandas-ta появляются без ручной регистрации) и ссылку на расширенный раздел API.
  - В `docs/user_guide/README.md` уточнить подпункт «Technical Analysis» (исправить ссылку и упомянуть LibraryManager как «простой способ» работы с внешними индикаторами).
- [ ] **Примеры / snippets**
  - Добавить краткий пример в `README.md` или `docs/examples/README.md` (в разделе про индикаторы) с демонстрацией вызова `LibraryManager.load_all_libraries()` и создания индикатора `pandas_ta` без дополнительного кода.
  - Убедиться, что пример в `examples/01_basic_indicators.py` или соседнем сценарии содержит ссылку на обновлённую документацию и при необходимости демонстрирует вызов LibraryManager.
