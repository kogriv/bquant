...
**Статус:** ✅ закрыт 2026-09-06 (AQ-033–038 сделаны; AQ-039 отклонён владельцем, §6).
...
## 6. AQ-039 — отклонено владельцем (2026-09-06)

Аудит предлагал разнести `analysis.zones` на `oscillator_zones/` и `price_levels/`. Решение
владельца: **не делать**. Зона в пакете — открытая абстракция (словарь типов объявляет
стратегия детекции, G28); аудит увидел в нём два частных случая и предложил закрепить их
в именах пакетов — после этого жить пришлось бы только в терминах осцилляторов и уровней,
а зоны бывают другими. Нарезка god-модулей за тем же импортом — отдельный вопрос, не
поднимался.
