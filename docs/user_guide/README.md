...
### ⌨️ [Командная строка](cli.md) - CLI `bquant`
- Команды `analyze` / `list` / `status`
- Выбор индикатора (`macd`, `rsi`, `ao`) и словари типов зон
- Структурный вывод `--json` для программ и ИИ-агентов
- Вызов тех же функций из кода
