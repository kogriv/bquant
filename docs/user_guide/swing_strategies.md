...
> **Для `find_peaks` и `pivot_points` этот режим сегодня обнуляет улов.** Он поднимает их
> `min_amplitude_pct` с `0.006` до `0.0186` и `0.0155`, тогда как медианный размах зоны на
> встроенных данных — `0.0122`: порог снова оказывается больше измеряемого. Замер на 83
> зонах MACD по гистограмме, пресет `narrow_zone`:
>
...
>
> Причина — поправка считается от размаха **всего датасета**, а применяется к движению
> **внутри зоны**; величины разного масштаба. Пока это не исправлено, включайте режим
> только с ZigZag. Разбор:
