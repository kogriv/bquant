...
Ключ складывается из пяти частей:

1. **Версия схемы** — поднимается, когда меняется форма результата.
2. **Хеш данных** — **весь кадр**: имена колонок, все колонки и индекс. До версии
   схемы 22 хешировались только `open/high/low/close`, и тот же OHLC с другим
   `RSI_14` или другим `volume` получал результат из кэша, посчитанный по чужой
   колонке (G53). Что именно читает стратегия, ключ знать не может, поэтому берёт всё.
...
5. **Подпись анализатора** — стратегии метрик и компоненты анализатора.
...
print(ZoneAnalysisCache.CACHE_VERSION)
```

Смена любого из перечисленного даёт **другой ключ**, и старые записи просто не находятся —
ручная очистка не нужна.

Подпись анализатора появилась в ключе в версии схемы 16. До неё стратегии метрик
`shape`, `divergence`, `volatility` и `volume` в ключ не входили вовсе, и включённая
стратегия получала результат прошлого прогона, посчитанный без неё, — без единого
предупреждения (G36). Записи, сделанные до 16-й версии, к чтению не принимаются.
...
| Ситуация | Действие |
| :--- | :--- |
| Изменение индикатора, детекции, любой стратегии, пресета или scope | **Не требуется** — новый ключ, новые записи |
