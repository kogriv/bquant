...
### Ключ декоратора `@cached` и функции `cache_key()`

Общий кэш функций строит ключ из версии пакета, имени функции и **канонической** записи
аргументов (SHA-256): кадры и `Series` — через `hash_pandas_object` вместе с именами
колонок и типами, массивы — байтами с формой и типом, словари — по отсортированным ключам,
примитивы — по `repr`. Ключ одинаков в разных процессах, поэтому дисковая запись
находится после перезапуска. До G66 (2026-09-06) здесь стояли `hash(str(arg))` и
`hash(bytes)`: Python солит их на процесс, и вызов со строкой, массивом или `Series`
получал в каждом процессе новый ключ — дисковый кэш для таких аргументов не переживал
перезапуск, хотя обещал. Чужой объект попадает в ключ своим `repr`; если тот несёт адрес
(`<... at 0x…>`), ключ снова нестабилен — это единственная оставленная дыра, и она
названа.

Запись, поднятая с диска в память, доживает **свой** срок: до G66 она получала новый
`default_ttl` (час) и могла пережить собственное истечение на диске.
...
## 9. Потоки и процессы: что обещано

Состояние, общее на процесс, названо здесь целиком (G64), чтобы обещание было
проверяемым, а не подразумеваемым:

| Что | Где | Контракт |
|---|---|---|
| кэш в памяти | `MemoryCache` | **потокобезопасен**: запись, порядок LRU и статистика под одним замком. До G64 восемь потоков на кэше из 40 записей давали 7–8 исключений за прогон и список порядка длиннее словаря |
| глобальный менеджер | `get_cache_manager()` | создаётся один раз за процесс, под замком; в тестах заменять через `bquant.core.cache._global_cache_manager` не нужно — есть `clear_cache()` |
| дисковый кэш | `DiskCache`, `~/.cache/bquant` | между **процессами** не координируется: два процесса могут писать один ключ; чтение и `clear()` терпят файл, удалённый соседом |
| реестры | индикаторы, детекторы, стратегии | заполняются при импорте (классы пакета) и при первой загрузке библиотек; менять после старта — на свой риск, замка нет |
| переопределения каталогов | `set_*_dir()` в `bquant.core.config` | на весь процесс, не на поток и не на контекст |
| флаги `BQUANT_SKIP_*` | окружение | читаются при первой загрузке библиотек; после неё не действуют |
