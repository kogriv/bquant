...
- **`.with_swing_preset()`** — с какими порогами. Пресетов два: `narrow_zone`
  (по умолчанию) и `wide_zone`. Имена говорят, под какую ширину зоны они
  откалиброваны, и разница огромна: на типичных MACD-зонах часового золота при
  `wide_zone` две стратегии из трёх не находят ничего.
...
Ноль в этой таблице сам по себе **неотличим** от честно измеренного отсутствия движения,
поэтому результат несёт `metadata['swing_coverage']`, а пустой улов у всех зон пишет
предупреждение. До 0.0.10 умолчанием был `wide_zone` (он назывался `default`) — и молчал.
Разбор:
