...
| **regression_results** | `Optional[Dict[str, Any]]` | нет | `ZoneRegressionAnalyzer` (predict_zone_duration, predict_price_return) → `RegressionResult.to_dict()` | Есть при `analyze(regression=True)` и числе измеренных зон > 10; при меньшем числе шаг не выполняется, и `metadata['regression']` сообщает `skipped` с причиной (G71). Ключи: `duration`, `return`; под каждым — словарь модели (`r_squared`, `coefficients`, `p_values`, `predictions`, `residuals`, `n_observations`, `metadata`) или `{'error': …}`, если подгонка не удалась. |
...
| **metadata** | `Dict[str, Any]` | да (по умолчанию `{}`) | Собирается в `UniversalZoneAnalyzer.analyze_zones()`; `validation` дописывает пайплайн | analysis_timestamp, total_zones, zone_types, clustering_performed, regression_performed, regression (`status`: `not_requested` / `executed` / `skipped` с `reason`, `zones_measured`, `zones_required`), duration_filter, validation (`status`); при наличии — swing_coverage, symbol, timeframe, source, dataset_name из `data.attrs`. **`regression_performed` отвечает «посчиталось ли», а на «просили ли» — только блок `regression`:** до G71 запрос при десяти зонах и меньше пропускался молча и выглядел как выключенный флаг. |
