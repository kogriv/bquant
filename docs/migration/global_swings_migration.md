...
- **Одна прогонка** стратегии вместо N запусков для каждой зоны. Выигрыша во времени это само по
  себе не даёт: в том же замере `zigzag` в глобальном режиме шёл 1.53 с против 1.08 с локально.
