...
## Слои смотрят в одну сторону

Порядок сверху вниз: `cli` → `visualization` → `analysis` → `indicators` → `data` → `core`.
Модуль импортирует только то, что **ниже** него, — и это относится к импортам внутри
функций тоже: ленивый импорт наверх — та же зависимость, только спрятанная. Корень
`bquant` вне порядка (в нём только `__version__` и реэкспорт конфигурации).

Сторожей два. `tests/unit/test_layers_point_one_way.py` разбирает импорты пакета через `ast`
и краснеет на ребре вверх, на приватном имени, импортированном из чужого модуля, и на
`pandas_ta` в `sys.modules` после `import bquant.analysis.zones`. Контракт для
[codemap](https://github.com/kogriv/codemap) — `codemap.toml`, секция `[architecture]`:
`codemap check --graph .codemap/graph.json` называет нарушенные рёбра по свежему графу.

До G64 (2026-09-06) рёбер наверх было два: `core.config` держал фабрики стратегий и лениво
импортировал реестр анализа, `data.schemas` создавала индикатор через фабрику. Оба
перенесены туда, где живёт знание, которое они использовали.
