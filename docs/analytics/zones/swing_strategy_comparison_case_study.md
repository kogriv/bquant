...
> в 0.0.10 и раньше этот режим поднимал `min_amplitude_pct` выше движения типичной зоны.
>
> **Дефект исправлен 2026-09-03 (G38), в отгруженном релизе его ещё нет.** Слой адаптирует только `deviation` у ZigZag, а
> `find_peaks` и `pivot_points` остаются на пороге пресета. Замер на 77 зонах:
>
> | Стратегия | режим выключен | было (0.0.10) | стало (в main) |
> |---|---|---|---|
