...
## ⚠️ Переоценка (2026-07): результат воспроизведён, но хрупок

Мера этого отчёта (`avg_rally_pct` vs `avg_drop_pct` из `SwingMetrics`, парный
односторонний Wilcoxon, `narrow_zone`, global) **воспроизводится бит-в-бит**:
histogram-зоны, `tv_xauusd_1h` → p=0.0084, покрытие 36/37. То есть отчёт посчитан
корректно. Но при перепроверке вскрылись существенные ограничения:

| База | Датасет | paired n | Wilcoxon p (rally>drop) |
|---|---|---|---|
| HISTOGRAM (MACD−signal) | 1h | 36 | **0.0084** ✓ |
| HISTOGRAM | m15 | 44 | 0.6464 ✗ |
| LINE (MACD>0) | 1h | 14 | 0.55 ✗ |
| LINE | m15 | 15 | 0.62 ✗ |

- **Single-dataset.** Отчёт гонялся только на `tv_xauusd_1h`. На `mt_xauusd_m15` та же
  мера незначима (p=0.65) — **репликации нет**.
- **Зависит от определения зоны.** Значимо только на зонах по **гистограмме**
  (MACD−signal). На зонах по **линии MACD** (методологически корректных, см.
  `gap_inventory_2026-07.md` G4) значимости нет ни на одной выборке.
- **Возможный артефакт паддинга.** `SwingMetrics` включает соседние ноги, в т.ч.
  rally, *входящую* в бычью зону (по построению это rally) — может завышать
  `avg_rally`. Требует пересчёта без входной ноги.

**Вывод:** «H1 подтверждена» следует читать как «значима на histogram-1h при
конкретном пресете и мере, не реплицирована, отсутствует на линии MACD». Дальнейшая
работа переходит на более устойчивую меру — экскурсию (MFE/MAE) от входа на откате с
обязательной репликацией 1h↔m15. См. `research/methodology/swing_structure_research_program.md`
(S1b) и `docs/analytics/zones/s1_swing_anatomy_case_study.md`.
