...
Индексы возвращаются из индекса кадра, счётчики — `int`. Эти четыре разреза —
`get_statistics`, `is_trending_up`, `is_trending_down`, `get_crossovers` — определены **один
раз**, на `BaseIndicator`, и есть у любого индикатора: до G65 (2026-09-06) они жили тремя
копиями, `get_crossovers` отдавал две разные формы ответа под одним именем, а у custom-MACD
его не было вовсе, хотя `get_info()` его рекламировал. Ошибка расчёта или колонка не из
выходов — исключение, а не `{}`/`False`.
