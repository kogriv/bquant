...
Своего кэша у индикатора нет: `calculate()` считает каждый раз. До G67 (2026-09-06) был
`calculate_with_cache()` — с тем же телом, что `calculate()`, и словом «кэш» в имени.
Кэшируется результат **анализа зон** целиком — [caching.md](../../user_guide/caching.md).
