...
> **Имена колонок пока прежние.** Этап C вводит адресацию по ролям, не меняя ни одного
> имени, чтобы каждый шаг оставался проверяемым против прежнего поведения. Перевод имён на
> слаги (`macd_12_26_9__hist`) — следующий шаг, вместе с переводом потребителей и
> документации.
