...
### `.with_atr_period(period)`

Период колонки `atr`, которую пайплайн добавляет в кадр, если её не принесли (по умолчанию 14;
ATR — скользящее среднее True Range, `calculate_atr()`). Её читают `atr_normalized_return` и
метаданные волатильности; период входит в ключ кэша. До 2026-09-05 пайплайн ждал `atr` от
функции, которая её не считала: ветка была недостижима, и `atr_normalized_return` был `None`
во всех зонах (G60).
...
Вывести пороги из самих данных вместо констант пресета. ZigZag получает `deviation` от
размаха кадра (на встроенном сэмпле `0.05` → `0.031`, покрытие не меняется). `find_peaks` и
`pivot_points` с G48 (2026-09-06) получают `min_amplitude_pct` от **масштаба зоны** —
`max(медианный размах бара, медианный размах зоны × k)`, k = 0.3 / 0.25; для этого пайплайн
детектирует зоны до глобальных свингов. На часовом сэмпле это 65 и 63 зоны из 77 против 28 и
38 у пресета, на 15-минутном — 67 и 72 из 91 против 3 и 3: константа пресета там стоит выше
размаха типичной зоны. Что из этого измерено и что нет —
[исследование G48](../../analytics/zones/swing_zone_scale_threshold_study.md).
...
### До расчёта: контракт входа

`build()` не начинает считать, если по данным нельзя честно посчитать метрики зон:

* **ось времени не по возрастанию или с повторами** — `ValueError` из
  `resolve_time_index()` с позицией разрыва или числом дублей и способом снять
  (`../data/processor.md`). До 2026-09-09 такой кадр доезжал тремя слоями ниже и падал
  на инварианте зоны сообщением «zone 5: start_time … > end_time …» — верным и
  бесполезным (G72);
* **пропуски в `open`/`high`/`low`/`close`** — `ValueError` со счётом по колонкам и
  указанием на `clean_ohlcv_data()`. Почему отказ, а не `NaN` в ответе: замер показал,
  что одна дыра ровно на границе зоны сдвигает её `price_return` с −0.000958 на
  −0.000651. `ewm` пропуск игнорирует, метрика считается от соседнего бара, и в
  результате нет ни `NaN`, ни предупреждения — только другое число. Валидатор данных
  (`validate_ohlcv_data`) отвечает на другой вопрос — «пригоден ли датасет вообще» — и
  малую долю пропусков терпит намеренно.

Что пайплайн **не** запрещает: разрывы в ряду (у рынка есть выходные) и ряд без
движения. Первое читается из результата — часов между границами зоны больше, чем в ней
баров; второе считается, а пустота свингов несёт причину `degraded='degenerate'` (G70).

### Во время расчёта: шаг, которому не хватило данных
