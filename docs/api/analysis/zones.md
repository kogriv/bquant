...
| `duration` | число баров; равно `end_idx - start_idx + 1`, и `data` несёт ровно столько строк (или ничего — у результата, загруженного без кадра). Зона, противоречащая себе, отвергается при создании (G66) |
