...
На встроенном сэмпле: 64 зоны без фильтра, 32 с ним. (Было 44 и 18, пока порог
длительности стоял в детекции с дефолтом `2` — разницу создавал он, а не данные.)
...
мостят область определения индикатора, и слой последовательностей читает соседние
элементы списка как переход. По умолчанию (`min_duration=1`) пропусков нет вовсе:
зоны стыкуются точно, и все `total_zones - 1` пар засчитываются. Если вы **просите**
отсев (`.analyze(min_duration=N)`), соседи выброшенной зоны перестают примыкать —
и такая пара переходом не является: между этими зонами было то, чего в выборке
нет. Пропуски считаются, а не замазываются.
...
На встроенном сэмпле по умолчанию (`min_duration=1`): MACD даёт 83 зоны, 82
засчитанных перехода, 0 отброшенных, покрытие 1000/1000 баров. С `min_duration=2`
те же данные дают 72 зоны, 63 засчитанных перехода и 8 отброшенных; пороговый RSI
(все три типа) — 30 и 13. Разница целиком создаётся фильтром.
