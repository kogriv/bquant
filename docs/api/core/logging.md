...
### `LoggingContext` - Операция как единица лога

Контекстный менеджер: пишет начало операции, а на выходе — успех с длительностью либо
ошибку. Любые именованные аргументы становятся контекстом и попадают в каждую строку
внутри блока.
...
Исключение внутри блока логируется и **пробрасывается дальше** — менеджер его не глотает.

### `log_function_call` - Декоратор трассировки вызовов

```python
from bquant.core import log_function_call
...
Пишет вход и успешный выход **на уровне DEBUG**, а ошибку — на уровне ERROR. При
стандартном уровне `INFO` в логе не появится ничего, пока функция не упадёт; это не
поломка декоратора, а его устройство. Чтобы увидеть трассировку, поднимите уровень:
`setup_logging(profile='debug')`.

### `log_performance` - Декоратор длительности

```python
from bquant.core import log_performance
...
Пишет строку **только если вызов занял больше секунды** (уровень INFO). Быстрый вызов
не логируется вовсе — молчание здесь означает «быстро», а не «не сработало». Ошибка
логируется всегда, вместе с временем до падения, и пробрасывается.

Порог зашит в декораторе и не настраивается. Если нужен полноценный замер — счётчики,
контекстный менеджер, история — это `bquant.core.performance`
([performance.md](performance.md)), а не эти декораторы.
