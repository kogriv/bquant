...
- `xaxis_num_ticks` (int, default=16): Желаемое количество меток на оси X (только для `dense` режима).
  **Пожелание, а не число меток:** фактическое = `max(8, min(запрос, ideal, точек))`, где `ideal`
  считается по длине и временному диапазону окна. Запрос ниже 8 не действует; на коротком окне
  (десятки баров) параметр не меняет ничего ни при каком значении.
- `show_gap_lines` (bool, default=`False`): Пунктирные вертикали там, где ось проглотила разрыв
  во времени (выходные, праздники) — в `timeseries` он вырезан `rangebreaks`, в `dense` ось
  позиционная, поэтому без линий разрыв не виден ни там, ни там. Порог — интервал больше 1.5
  медианных. Только бэкенд plotly.
...
Границы можно давать и наивными, и с часовым поясом: если у данных пояс есть
(у встроенных сэмплов это `UTC+07:00`), наивные границы локализуются в него. До
сентября 2026 наивная граница давала голый `TypeError: can't compare offset-naive and
offset-aware datetimes` — сообщение, по которому неясно, что чинить.
...
Блок печатается **для каждого встреченного типа зон**. На встроенном сэмпле
(`tv_xauusd_1h`, стратегия свингов `zigzag`) режим `compact` даёт:

```
📊 Bullish Zones: 39/39 with swings (100%)
  Avg Rally: +0.54% ± 0.23%
  Avg Drop: +0.43% ± 0.22%
  Mean per-zone Rally/Drop: 1.33x
📊 Bearish Zones: 38/38 with swings (100%)
  Avg Rally: +0.45% ± 0.21%
  Avg Drop: +0.50% ± 0.24%
  Mean per-zone Rally/Drop: 1.94x
```

Режим `full` добавляет к тем же строкам длительности в барах.

**Отношение считается не из двух строк над ним.** `Mean per-zone Rally/Drop` — это
среднее **по зонам** от отношения rally к drop внутри каждой зоны, а не частное
показанных средних: у медвежьих зон это 1.94x при том, что 0.45/0.50 = 0.90. Обе
величины законны и отвечают на разные вопросы; до сентября 2026 строка называлась
`Rally/Drop Ratio` и читалась как частное соседних чисел.

**`Avg Drop` печатается со знаком плюс** — это амплитуда движения, а не знаковая
доходность.
...
**Агрегация — среднее ± стандартное отклонение.** Медиана, межквартильный размах и
метрики формы не считаются; заявка на них лежит в
`devref/gaps/graph/zomet_v1.2_advanced_aggregation.md`.
...
- `xaxis_num_ticks` (int, default=16): Желаемое количество меток на оси X (для `dense` режима); зажимается, см. выше.
...
- **Умные пути**: Автоматически сохраняет в `outputs/vis/<имя_скрипта>/<имя_файла>`.
