...
| правило нечего применять (нет значений) | `warnings`; для **обязательного** поля, пустого целиком, — `issues` |
| в кадре нет строк | `issues`, `is_valid=False`: колонки без строк — не данные (`min_rows`, по умолчанию 1) |
| нарушено отношение между колонками (`high < low`, `high < max(open, close)`, `low > min(open, close)`) | `issues` + счётчик по правилу в `stats['frame_violations']`; правила добавляются `add_frame_rule(name, df -> число строк)` |

Три отношения OHLC — одна функция `ohlc_violations(df)` для схемы, `validate_ohlcv_data()`
и загрузчика. До 2026-09-05 схема их не проверяла вовсе, валидатор смотрел только
`high < low`, а полный набор знала одна `OHLCVRecord.validate()` — один контракт с
четырьмя строгостями (G57).
