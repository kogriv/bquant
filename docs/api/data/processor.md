...
### Что `resolve_time_index()` отвергает

Функция владеет осью времени, поэтому здесь же проверяется то, на что опирается всё
ниже по течению — срезы зон по индексу, границы `start_time`/`end_time`, ключ кэша:

| вход | ответ |
|---|---|
| колонка времени с `NaT` | `ValueError`: сколько значений не распарсилось и примеры (G57) |
| ось не по возрастанию | `ValueError`: позиция первого разрыва и способ — `df.sort_index()` |
| повторы по времени | `ValueError`: сколько дублей, примеры и `df[~df.index.duplicated(keep='last')]` |
| времени в кадре нет вовсе | кадр возвращается как есть, индекс остаётся позиционным (G30) |

До 2026-09-09 неотсортированный кадр и дубли доезжали **тремя слоями ниже** и падали на
инварианте зоны сообщением «zone 5: start_time … > end_time …»: верным и бесполезным —
оно называет следствие, а причину (кадр пришёл вперемешку) не упоминает вовсе (G72).
