...
`expected_frequency` на определение не влияет, он используется для сверки. Задаётся он
в соглашении проекта (`'15m'` — пятнадцать минут, `'1h'`, `'1d'`) или pandas-алиасом
(`'15min'`, `'D'`): строка переводится через `pandas_offset_alias`, а не уходит в pandas
как есть, где `m` означало бы месяц.
