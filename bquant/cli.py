...
#:
#: v2 (G34): ``total_statistics`` проходит сюда как есть, и его форма изменилась.
#: Появились ``zones_by_type``/``ratios_by_type`` — распределение по фактически
#: встреченным типам; ``bull_*``/``bear_*`` остались только там, где эти типы есть.
#: Раньше они стояли всегда и на не-MACD осцилляторе печатали нули при непустом
#: наборе зон.
