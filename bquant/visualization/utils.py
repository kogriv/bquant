...
def generate_dense_axis_labels(
    ...
    Args:
        ...
        num_ticks_requested: Желаемое количество меток (по умолчанию 16).
                            **Пожелание, а не число меток.** Фактическое значение —
                            ``max(8, min(num_ticks_requested, ideal, data_points))``,
                            где ``ideal`` считается по длине и временному диапазону
                            окна. Запрос ниже 8 не действует никогда; на коротком окне
                            (десятки баров) ``ideal`` сам равен 8, и тогда параметр
                            не меняет ничего ни при каком значении
