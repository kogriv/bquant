...
class StatisticalAnalyzer(BaseAnalyzer):
    ...
    def descriptive_statistics(self, data: pd.Series, name: str = "data") -> Dict[str, float]:
        ...
        Args:
            ...
            name: Метка **только для отладочной строки в логе**. В результат не
                попадает и на него не влияет: возвращаемый тип — ``Dict[str, float]``,
                строке в нём места нет. Если нужно подписать результат, подписывайте
                его у себя.
...
def get_statistical_analyzers() -> Dict[str, str]:
    """
    Виды статистического анализа, которые покрывает модуль.
